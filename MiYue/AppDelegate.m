...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    //设置跟视图控制器
    EquipmentListViewController *EquipmentListPageVc = [[EquipmentListViewController alloc]init];
    
    UINavigationController *navCtrlr = [[UINavigationController alloc]initWithRootViewController:EquipmentListPageVc];
    
    self.window = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];//UIScreen mainScreen].bounds个人理解获取屏幕的硬件信息然后设置尺寸大小
    self.window.rootViewController = navCtrlr;//设置根视图控制器
